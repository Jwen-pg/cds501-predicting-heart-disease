...
## Class Distribution after removing outlier

```{r}
ggplot(data) + geom_bar(aes(x = Target, fill = Target), alpha=0.8) + theme_bw() + ggtitle("Class Distribution") + xlab("Target") + ylab("Count") + scale_fill_manual(values=c("#15158a", "#eb4034"))

# saves plot
png(file="../figures/class_dist_removed_outlier.png", width=600, height=350)
ggplot(data) + geom_bar(aes(x = Target, fill = Target), alpha=0.8) + theme_bw() + ggtitle("Class Distribution") + xlab("Target") + ylab("Count") + scale_fill_manual(values=c("#15158a", "#eb4034"))
dev.off()
...
```{r}
preProcValues <- preProcess(x = training_set, method = c("center", "scale"))
set.seed(400)
...
svm <- train(Target ~ ., data = data, method = "svmLinear3", trConrol = lr_ctrl)
```
...
```{r}
tree_fit <- rpart(Target ~  Systole + Cholesterol + Age + Diastole + Weight + BMI + BloodPressure + Active, data = training_set, control = rpart.control(minsplit = 1000), xval = 5)
